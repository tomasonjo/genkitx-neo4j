...
const documents = [Document.fromText("Tomaz je kralj", {wtf: "wtf"}), Document.fromText("Kralj je Tomaz", {wtf: "wtf"})]
indexy();
...
const queryy = async () => {
  const data = await ai.retrieve({ retriever: bobFactsRetriever, query,     options: {
